...
error_reporting(E_ALL & ~E_NOTICE); //Noticeを表示させない
 ...
 //変数
    ...
    if(file_exists($filename)){
        ...
        $postnum=1;
//データベースへの接続 m4-1
	$dsn = 'mysql:dbname=tb220293db;host=localhost';
	$user = 'tb-220293';
	$password = 'j8LvEDVY6p';
	$pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
//テーブル作成に関する処理 m4-2
	$sql = "CREATE TABLE IF NOT EXISTS tbrina"
	." ("
...
//yes投稿に関する処理 mission4-5,4-7
    $sql = $pdo -> prepare("INSERT INTO tbrina (name, comment) VALUES (:name, :comment)");
...
//削除
    if(!empty($deletenum) && !empty($_POST["delete"]) && !empty($delpass)){//もし削除対象番号、削除ボタン、削除パスワードが押されているとき
        $sql = 'SELECT * FROM tbrina';
        $stmt = $pdo->query($sql);
        $results = $stmt->fetchAll();
        foreach ($results as $row){
            if($row['password']==$delpass){ //削除
                $sql = 'delete from tbrina where id=:id';
                $stmt = $pdo->prepare($sql);
                $stmt->bindParam(':id', $deletenum, PDO::PARAM_INT); //id＝deletenum
                $stmt->execute();
                $del="削除しました";
            }else{
...
if(!empty($editnum) && !empty($_POST["edit"]) && !empty($editpass)){
 ...
 if (!empty($editNO)){
        if(!empty($name) && !empty($str) && !empty($pass)) {
            $id=$editNO;
            $sql = 'UPDATE tbrina SET name=:name,comment=:comment,date=:date,password=:password WHERE id=:id';
            $stmt = $pdo->prepare($sql);
...
<body>
    ...
    <form action="" method="post">
        ...
        <?php  if(isset($_POST["edit"])){echo $editname;} ?>"> 
           <input type="text" name="str" placeholder="コメント" value="
        <?php if(isset($_POST["edit"])){echo $editcomment;} ?>"> </p>
        <p><input type="password" name="pass" placeholder="パスワード" value="
        <?php  if(isset($_POST["edit"])){echo $passedit;} ?>">
           <input type="submit" name="submit" value="送信"></p>
        <p><input type="hidden" name="editNO" value="
        <?php if(isset($_POST["editnumber"])) {echo $editnumber;} ?>"></p>
